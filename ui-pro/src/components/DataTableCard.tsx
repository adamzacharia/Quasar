...
export function DataTableCard({ data }: DataTableCardProps) {
    ...
    return (
        <div className="bg-sidebar-dark border border-slate-700/50 rounded-2xl overflow-hidden shadow-sm">
            ...
            {/* ── Table — horizontally scrollable if needed, vertically capped ── */}
                            ...
                            {rows.map((row, ri) => (
                                ...
                                >
                                    {columns.map((col, ci) => {
                                        const val = row[col];
                                        const isFirst = ci === 0;
                                        const display = val === null || val === undefined || val === "nan" || val === "None"
                                            ? "—"
                                            : String(val);
                                        return (
                                            <td
                                                key={ci}
                                                title={display}
                                                className={`py-2.5 px-4 text-xs font-mono max-w-[200px] truncate ${
            ...
            {/* ── Footer ── */}
            <div className="flex items-center justify-between gap-3 px-4 py-3 bg-surface-dark border-t border-slate-700/50">
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                    {data.archiveLink && (
                        ...
                        >
                            <ExternalLink className="w-3.5 h-3.5" />Open in ALMA Archive
                        </a>
                    )}
                    <button className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:bg-slate-700 rounded-lg transition-colors">
                        <Download className="w-3.5 h-3.5" />Download CSV
                    </button>
                    <button className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-white bg-primary hover:bg-primary/90 rounded-lg shadow-md shadow-primary/20 transition-colors">
                        <BarChart3 className="w-3.5 h-3.5" />View Plots
                    </button>
                </div>
            </div>
        </div>
