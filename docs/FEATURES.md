...
- [x] **R11** ALMA QA2 Status Display -- QA2 PASS/SEMIPASS is read from ALMA archive table data (`qa2_passed`) and displayed in result tables without parsing report PDFs.
...
- [x] **PS42** User Provider Key Storage & Platform Usage Quotas -- Implemented Bring Your Own Key (BYOK) database storage with Fernet encryption, custom token usage limits, admin exemptions, and real-time secret redaction in error traces. (services/provider_key_service.py + services/usage_quota_service.py + services/secret_redaction.py)
