...
class FITSProcessingService:
    ...
    def extract_metadata_from_url(url: str) -> Dict[str, Any]:
        """
        Read FITS header metadata from a remote URL without downloading the
        full file.  Uses fsspec for lazy HTTP range-request access.

        Extracts key science-relevant headers:
          - BMAJ / BMIN / BPA (beam geometry, converted to arcsec)
          - RMS / DATARMS / NOISE (sensitivity)
          - RESTFRQ (rest frequency in GHz)
          - OBJECT (target name)
          - NAXIS1 / NAXIS2 / CDELT1 / CDELT2 (image dimensions & pixel scale)
          - TELESCOP / INSTRUME / DATE-OBS

        Parameters
        ----------
        url : str
            HTTPS URL pointing to a FITS file (e.g. from ALMA DataLink).

        Returns
        -------
        dict
            Extracted metadata, or {"error": "..."} on failure.
        """
        if not ASTROPY_AVAILABLE:
            return {"error": "Astropy required for remote FITS header reading"}

        try:
            # Use fsspec for lazy HTTP access — only reads header blocks
            with fits.open(
                url,
                use_fsspec=True,
                lazy_load_hdus=True,
                ...
                # Try primary HDU first, then image extensions
                header = hdul[0].header
                if header.get("NAXIS", 0) == 0 and len(hdul) > 1:
                ...
                # ── Beam parameters (degrees → arcsec) ────────────────
                bmaj_deg = header.get("BMAJ")
                bmin_deg = header.get("BMIN")
                bpa = header.get("BPA")

                if bmaj_deg is not None:
                    metadata["BMAJ_arcsec"] = round(bmaj_deg * 3600, 4)
                if bmin_deg is not None:
                    metadata["BMIN_arcsec"] = round(bmin_deg * 3600, 4)
                if bmaj_deg is not None and bmin_deg is not None:
                    metadata["beam_summary"] = (
                        f"{metadata['BMAJ_arcsec']:.4f}\" × "
                        f"{metadata.get('BMIN_arcsec', '?')}\" "
                        f"(PA={bpa}°)" if bpa is not None else
                        f"{metadata['BMAJ_arcsec']:.4f}\" × "
                        f"{metadata.get('BMIN_arcsec', '?')}\""
                    )
                if bpa is not None:
                    metadata["BPA_deg"] = bpa

                # ── Sensitivity / RMS noise ────────────────────────────
                for rms_key in ("RMS", "DATARMS", "NOISE", "ACTNOISE"):
                    val = header.get(rms_key)
                    if val is not None:
                        metadata["RMS"] = val
                        metadata["RMS_key"] = rms_key
                        break

                # ── Rest frequency (Hz → GHz) ─────────────────────────
                restfrq = header.get("RESTFRQ") or header.get("RESTFREQ")
                if restfrq is not None:
                    metadata["RESTFRQ_Hz"] = restfrq
                    metadata["RESTFRQ_GHz"] = round(restfrq / 1e9, 6)

                # ── Pixel scale ────────────────────────────────────────
                cdelt1 = header.get("CDELT1")
                cdelt2 = header.get("CDELT2")
                if cdelt1 is not None:
                    metadata["CDELT1_arcsec"] = round(abs(cdelt1) * 3600, 6)
                if cdelt2 is not None:
                    metadata["CDELT2_arcsec"] = round(abs(cdelt2) * 3600, 6)
            ...
            logger.error("Error reading remote FITS header from %s: %s", url, e)
